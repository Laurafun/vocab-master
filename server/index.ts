...
const envPath = path.join(__envDirname, '..', '.env');
...
import { handleCorrect, handleWrong, getBoxDescription, getBoxColor, FORGETTING_CURVE_DATA, SPACED_REPETITION_EFFECT } from "./spaced-repetition.js";
...
app.get("/api/words", (req, res) => {
  try {
    ...
    res.status(500).json({ error: error?.message || "获取单词失败" });
  }
});
...
app.post("/api/words", (req, res) => {
  try {
    const { word, phonetic, definition, example, example_translation, tags } = req.body;
    if (!word || !definition) {
      return res.status(400).json({ error: "单词和释义不能为空" });
    }
    // 检查是否已存在
    const existing = db.getWordByText(word);
    if (existing) {
      return res.status(409).json({ error: `单词 "${word}" 已存在`, existing });
    }
    const newWord = db.createWord({ word, phonetic, definition, example, example_translation, tags });
  ...
  try {
    ...
    for (const w of words) {
      if (!w.word || !w.definition) {
        return res.status(400).json({ error: "每个单词需要包含 word 和 definition" });
      }
    }
    const result = db.createWords(words);
    res.json(result);
  } catch (error: any) {
    console.error("[Batch Create Words] Error:", error);
    res.status(500).json({ error: error?.message || "批量添加单词失败" });
  }
});
...
app.put("/api/words/:wordId", (req, res) => {
  try {
    ...
    const { word, phonetic, definition, example, example_translation, tags } = req.body;
    const success = db.updateWord(wordId, { word, phonetic, definition, example, example_translation, tags });
    if (!success) {
    ...
    res.status(500).json({ error: error?.message || "更新单词失败" });
  }
});
...
app.delete("/api/words/:wordId", (req, res) => {
  try {
    const { wordId } = req.params;
    const success = db.deleteWord(wordId);
    if (!success) {
      return res.status(404).json({ error: "单词不存在" });
    }
    res.json({ success: true });
    ...
    res.status(500).json({ error: error?.message || "删除单词失败" });
  }
});
...
app.get("/api/study/today", (req, res) => {
  try {
    const dailyLimitStr = db.getSetting("daily_limit") || "20";
    const dailyLimit = parseInt(dailyLimitStr, 10);

    // 获取到期需要复习的单词（包括新单词和之前学过需要复习的）
    const dueWords = db.getDueWords(dailyLimit);

    // 如果到期单词不足，补充新单词
    let result = dueWords;
    if (result.length < dailyLimit) {
      const newWords = db.getNewWords(dailyLimit - result.length);
      // 去重
      const existingIds = new Set(result.map(w => w.id));
      for (const nw of newWords) {
        if (!existingIds.has(nw.id)) {
          result.push(nw);
        }
      }
    }
    ...
    for (const w of result) {
      if (w.status === 'new') {
        db.updateWord(w.id, { status: 'learning' });
        w.status = 'learning';
      }
    }

    const stats = db.getTodayReviewStats();
    ...
    res.status(500).json({ error: error?.message || "获取今日单词失败" });
  }
});
...
app.post("/api/study/review", (req, res) => {
  try {
    ...
    if (!wordId || !result || !['correct', 'wrong'].includes(result)) {
      return res.status(400).json({ error: "参数错误：需要 wordId 和 result (correct/wrong)" });
    }
    ...
    const levelBefore = word.box_level;
    let levelAfter: number;
    let nextReview: string;
    let status: string;

    if (result === 'correct') {
      const handled = handleCorrect(levelBefore);
      levelAfter = handled.newLevel;
      nextReview = handled.nextReview;
      status = handled.status;
    } else {
      const handled = handleWrong(levelBefore);
      levelAfter = handled.newLevel;
      nextReview = handled.nextReview;
      status = handled.status;
    }
    ...
    db.updateWord(wordId, {
      box_level: levelAfter,
      next_review: nextReview,
      last_reviewed: new Date().toISOString(),
      review_count: word.review_count + 1,
      correct_count: result === 'correct' ? word.correct_count + 1 : word.correct_count,
      wrong_count: result === 'wrong' ? word.wrong_count + 1 : word.wrong_count,
      status,
    });
    ...
    db.createReviewRecord({
      word_id: wordId,
      result,
      box_level_before: levelBefore,
      box_level_after: levelAfter,
    ...
    res.json({
      ...
      nextReview,
      status,
      boxDescription: getBoxDescription(levelAfter),
      boxColor: getBoxColor(levelAfter),
    });
  } catch (error: any) {
    console.error("[Study Review] Error:", error);
    res.status(500).json({ error: error?.message || "提交复习结果失败" });
  }
});
...
app.get("/api/stats", (req, res) => {
  try {
    const stats = db.getWordStats();
    const todayStats = db.getTodayReviewStats();
    const recentStats = db.getRecentReviewStats(30);

    // 计算连续学习天数
    let streak = 0;
    for (let i = recentStats.length - 1; i >= 0; i--) {
      if (recentStats[i].total > 0) {
        streak++;
      } else {
        break;
      }
...
app.get("/api/stats/curve", (req, res) => {
  try {
    const stats = db.getWordStats();
    const boxDistRaw = db.getBoxDistribution();
    // 各 Box 等级的单词分布
    const boxDistribution = boxDistRaw.map(item => ({
      level: item.level,
      count: item.count,
      description: getBoxDescription(item.level),
      color: getBoxColor(item.level),
    }));

    res.json({
      forgettingCurve: FORGETTING_CURVE_DATA,
      spacedRepetitionEffect: SPACED_REPETITION_EFFECT,
      boxDistribution,
      totalWords: stats.total,
    });
  } catch (error: any) {
    console.error("[Curve] Error:", error);
    res.status(500).json({ error: error?.message || "获取曲线数据失败" });
  }
});
...
app.get("/api/settings", (req, res) => {
  try {
    ...
    // 提供默认值
    const result = {
      daily_limit: parseInt(settings.daily_limit || "20", 10),
      auto_play_audio: settings.auto_play_audio === "true",
      show_phonetic: settings.show_phonetic !== "false",
      show_example: settings.show_example !== "false",
...
app.put("/api/settings", (req, res) => {
  try {
    const { daily_limit, auto_play_audio, show_phonetic, show_example, theme } = req.body;
    
    if (daily_limit !== undefined) {
      const limit = parseInt(daily_limit, 10);
      if (isNaN(limit) || limit < 10 || limit > 60) {
        return res.status(400).json({ error: "每日单词数应在 10-60 之间" });
      }
      db.setSetting("daily_limit", String(limit));
    }
    if (auto_play_audio !== undefined) {
      db.setSetting("auto_play_audio", String(auto_play_audio));
    }
    if (show_phonetic !== undefined) {
      db.setSetting("show_phonetic", String(show_phonetic));
    }
    if (show_example !== undefined) {
      db.setSetting("show_example", String(show_example));
    }
    if (theme !== undefined) {

...
CodeBuddy Code 支持多种权限模式，可在 [settings](settings.md) 的 `permissions.defaultMode` 中配置，也可通过 `--permission-mode` 指定。常用模式如下：
...
更完整的模式语义、切换方式、状态栏提示和子代理继承规则，请直接参考 [权限模式](permission-modes.md)。

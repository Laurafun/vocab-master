/**
 * CodeBuddy Node safe-delete shim
 *
 * Loaded by node-language-shim.cjs. Installs only delete-protection hooks.
 */
...
const SAFE_DELETE_ENABLED = process.env.CODEBUDDY_SAFE_DELETE_ENABLED !== '0';

if (!SAFE_DELETE_ENABLED) {
    return;
}

if (global.__CODEBUDDY_NODE_SAFE_DELETE_SHIM_LOADED__) {
    return;
}
global.__CODEBUDDY_NODE_SAFE_DELETE_SHIM_LOADED__ = true;
...
function normalizePathForCompare(filePath) {
    ...
    try {
        ...
        // Non-existent paths still need stable comparison for native error passthrough.
    }
...
function shouldBypassSafeDelete(absPath) {
...
// ---------------------------------------------------------------------------
// broker IPC delete：优先走 HostFileOperation delete → host service → trash
// ---------------------------------------------------------------------------

function tryBrokerDelete(absPath, opts) {
    let deleteBroker;
    try {
        deleteBroker = require('./safe-delete-broker-delete.cjs');
    } catch (_) {
        return 'unavailable';
    }
    return deleteBroker.tryBrokerDelete(absPath, {
        runtime: 'node',
 ...
 * @param {string} absPath 绝对路径
 */
function tryTrash(absPath, opts) {
    checkBulkDeleteGuard(absPath);

    const brokerResult = tryBrokerDelete(absPath, opts);
    if (brokerResult === 'ok') {
        return true;
    }
    if (brokerResult === 'denied') {
        throw new Error(`[safe-delete] broker denied delete: path=${absPath}`);
    }
...
function makeSyncWrapper(orig, requireDir) {
    return function (filePath, ...rest) {
        ...
        if (shouldBypassSafeDelete(absPath)) return orig(filePath, ...rest);
        if (requireDir) {
        ...
        tryTrash(absPath, rest[0] && typeof rest[0] === 'object' ? rest[0] : undefined); // 失败时抛错，不调 orig（fail-closed）
        return undefined;
    };
}
...
function makeCallbackWrapper(orig, requireDir) {
    return function (filePath, ...rest) {
        ...
        try {
            tryTrash(absPath, opts || undefined);
        } catch (e) {
            if (typeof cb === 'function') {
...
function makePromiseWrapper(orig, requireDir) {
    return async function (filePath, ...rest) {
        ...
        tryTrash(absPath, opts || undefined); // 失败抛错 → async 函数自动转 rejected Promise（fail-closed）
        return undefined;
    };
}
...
function tryRm(absPath, opts) {
    if (shouldBypassSafeDelete(absPath)) {
        return { done: false };
    }
    ...
    tryTrash(absPath, opts); // 失败时抛错（fail-closed）
    return { done: true };
}
